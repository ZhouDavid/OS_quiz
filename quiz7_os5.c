...
char pg_mem[20 * 4096]; // page dir + 16 kernel entries + 1 user entry + alignment

int *pg_dir, *pg_tbl[16], *pg_tbl_usr;
...
// char pg_mem[20 * 4096]; // page dir + 4 entries + alignment
// int *pg_dir;
// int pg[PG_NUM];
...
setup_user_paging()
{
  ...
  int i, j, temp;
  int PG_DIR_OFFSET = ((unsigned int)USRSTART) >> 22;

  // set up pointer
  pg_dir = (int *)((((int)&pg_mem) + 4095) & -4096);
  pg_tbl_usr = pg_dir + 17 * 1024;
  
  // set up pg_dir
  pg_dir[PG_DIR_OFFSET] = (int)pg_tbl_usr | PTE_P | PTE_W | PTE_U;

  // set up pg_tbl
  for(i = 0; i < 1024; i++){
    pg_tbl_usr[i] = 0;
  }
  for(i = 0; i < (((unsigned int)(USREND - USRSTART)) >> 12); i++){
    pg_tbl_usr[i] = (int)((i << 12) + USRPHY) | PTE_P | PTE_W | PTE_U;
  }
...
setup_kernel_paging()
{
  int i, j, temp;
  int PGT_NUM = 16;
  unsigned int PG_DIR_OFFSET = ((unsigned int)KERSTART) >> 22;

  // set up pointer
  pg_dir = (int *)((((int)&pg_mem) + 4095) & -4096);
  for(i = 0; i < PGT_NUM; i++){
    pg_tbl[i] = (i == 0 ? pg_dir : pg_tbl[i-1]) + 1024;
  }

  // set up pg_dir
  for(i = 0; i < 1024; i++) pg_dir[i] = 0;
  for(i = 0; i < PGT_NUM; i++){
    pg_dir[i + PG_DIR_OFFSET] = (int)pg_tbl[i] | PTE_P | PTE_W;  // map 64M
  }
  pg_dir[0] = pg_dir[PG_DIR_OFFSET];

  // set up pg_tbl
  for(i = 0; i < PGT_NUM; i++){
    for(j = 0; j < 1024; j++){
      temp = i * 1024 + j;
      pg_tbl[i][j] = (temp<<12) | PTE_P | PTE_W;
    }
  }
}
